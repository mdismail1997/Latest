...
export const removeItemData = createAsyncThunk(
  ...
  async id => {
    try {
      const responseData = await authService.removeItemCart(id);
      console.log('removeItem success', responseData.status);

      if (responseData.data.message == 'User not found') {
        toasterr.showToast('For further processing, please log in or register');
      } else {
        toastr.showToast('Item removed successfully');
      }
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(removeItemData.fulfilled, (state, action) => {
        ...
        state.loading = false;
        state.removeItemdata = action.payload;
      })
