...
export default class BuyForMe extends React.Component {
    keyboardheight = 0;
    amount = 0;
    keyboardScreenY = 0;
    inputheight = 60;
    ...
    render() {
        return (
            <ScrollView style={styles.container} ref="scrollviewbuy" keyboardShouldPersistTaps='always'>
                <GiftAmount onPay={async () => {await this.onPay()}} provider={this.props.provider} onInputAmount={this.inputAmount} language={this.props.language} token={this.props.token} clients={this.props.clients} type="buyforme" />    
            </ScrollView>
        );
    }
}
